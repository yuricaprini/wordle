...
class RankingTest {
  ...
  void testGetFromUpTo() {
    ...
    ArrayList<Pair<Username, Score>> result = ranking.getBottomUp(10);

    System.out.println("getBottom");
    for (Pair<Username, Score> pair : result) {
      System.out.println(pair.getValue0().toString() + " " + pair.getValue1().getValue());
    }

    ranking.update(new Username("User3"), new Score(120), new Score(151));

    // Retrieve users with scores greater than 100
    result = ranking.getBottomUp(new Username("User3"), new Score(120), 10);

    System.out.println("getBottomUp");
    for (Pair<Username, Score> pair : result) {
      System.out.println(pair.getValue0().toString() + " " + pair.getValue1().getValue());
    }
    // //Verify the result
